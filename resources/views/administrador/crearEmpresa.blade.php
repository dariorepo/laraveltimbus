...
<body>
 ...
 <article class="principal d-flex justify-content-center">
    ...
    <form class="form_reg" action="{{Route('crear_Empresa')}}" method="POST" enctype="multipart/form-data">
        {{--  --}}

        <h2>Crear Una Nueva Empresa</h2>
        ...
        <label for=""> Consultar Ruta </label><br>
        <select name="id_ruta">

            @foreach ($rutas2 as $ruta)
            <option value="{{$ruta->id}}"> {{$ruta->nombre}} {{$ruta->horario}}</option>
            @endforeach

        </select><br><br>


        <button type="submit" class="btn btn-success">Crear Empresa</button><br>
